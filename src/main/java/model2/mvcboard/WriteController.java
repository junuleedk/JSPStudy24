...
/*
  글쓰기의 경우 <a>태그의 링크를 통해 진입할 때는 get방식, 작성처리를 위해서는 post방식의 전송을
  해야하므로 doGet, doPost 모두 필요하다.
 */
public class WriteController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		try {
			...
			/*
			 파일업로드시 오류가 발생되면 경고창을 띄운 후 작성페이지로 이동한다. 예외 발생의 이유를 확인하기 위해
			 printStackTrace()매서드를 사용하는 것이 좋다. 
			 */
			e.printStackTrace()
			//업로드 오류시 경고창을 띄우고 쓰기 페이지로 이동한다.
