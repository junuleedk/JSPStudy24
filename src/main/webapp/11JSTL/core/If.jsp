...
<body>
	...
	<!-- 
		Java에서는 비교연산자 ==과 equals()가 다르지만 EL에서는 eq를 통해 값에
		대한 비교와 문자열에 대한 비교 모두를 할 수 있다. 
		여기서 사용한 eq는 Java의 compareTo()와 같이 사전순으로 문자열을 비교한다.
	 -->
	 
	<!--비교하는 문자열이 서로 다르므로 false가 result2에 저장된다.-->
	<c:if test="${ string eq 'Java' }" var="result2">
		문자열은 Java입니다. <br/>
	</c:if>
	<!-- result2에 not를 붙여 반대의 조건을 만들어 else와 같은 구문을 만들어준다. -->
	...
	<!-- 
		test 조건을 추가할 때 앞뒤로 공백이 하나라도 들어가면 무조건 false를 반환하므로
		작성시 주의해야한다.
	 -->
...
<h4>연습문제 : if태그</h4>
	...
	<!-- 최초 실행시에는 파라미터가 없는 상태이므로 아무내용도 출력되면 안된다
		 전송된 파라미터가 있을때만 보여지도록 해야한다.
	 -->
    ...
    <p>${param.user}님, 하이룽~</p>
	</c:if>

	<c:if test="${not (param.user eq 'tjoeun' and param.pass eq '1234')}">
    <p>아이디/비번을 확인하세요</p>
	</c:if>
	-->
</body>
