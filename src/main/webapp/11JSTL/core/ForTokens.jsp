...
<head>
<meta charset="UTF-8">
<title>JSTL - forTolkens</title>
</head>
<body>
	<!-- 
		forTokens 태그 : 문자열에서 특정 구분자를 통해 반환된 배열의 크기만큼
						반복할 때 사용한다.
				items : 구분자를 포함한 문자열을 설정한다. 단, 컬렉션이나 배열은
						사용할 수 없다.
				delims(delimeter) : 구분자를 지정한다. 여러가지 특수기호를
									사용할 수 있다.
				var : 구분자를 통해 잘라 낸 토큰을 저장한다.
				
		※토큰이란? : 문법적으로 의미 있는 최소단위를 말한다. 하이픈으로 구분되어있는
				   전화번호를 분리했을때 각각의 번호가 토큰이된다.
	 -->
	<%
	//콤마로 구분된 문자열을 정의한다.
