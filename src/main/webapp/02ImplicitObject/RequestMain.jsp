...
2. post방식
  ...
  <!--GET(주소줄에 노출됌) 방식으로 요청-->
  <!-- 
  <a>태그를 통해 페이지 이동시 파라미터를 전달하고 싶다면 파일명 뒤에 물음표를 붙이고 "파라미터=값"의 형태로 작성한다.
  2개 이상의 파라미터는 &를 통해 구분하면된다. 이 부분을 "쿼리스트링(Query string)"이라고 한다.
   -->
    ...
    <br />
  <!--POST(주소줄에 노출안됌) 방식으로 요청-->
  <!--
  <form>태그 하위의 <input>, <select>, <textarea>태그를 통해 값을 전송할 때는 반드시 name속성이 명시되어야한다.
  name속성에 지정한 이름을 서버로 파라미터를 전송할 수 있다.
    -->
