...
/*
문자열로 전송되는 값을 실제 날짜로 변경하기 위해 포맷을 지정한 후 타임스템프로 변경한다.
초단위로 변경된 시간은 long타입의 변수에 저장한다.
*/
...
/*
addDateHeader()
	: 응답헤더에 날짜형식을 추가하는 경우 long타입의 타임스템프로 변환 후 추가한다.
addIntHeader()
	: 숫자형식의 응답헤더를 추가한다.
addIntHeader()
	: 문자형식의 응답헤더를 추가한다.
...
<body>
	...
	/*
	출력결과에서 날짜의 경우 하루전인 11월 30일이 출력된다. 이것은 대한민국이 세계표준시(그리니치천문대)에
	+09 즉 9시간이 빨라서 생기는 현상으로 9시간을 더해줘야 정상적인 날짜가 출력된다. 
	
	myNumber라는 헤더명이 2번 출력되는데 8282만 나오는걸 볼 수 있다. 이것은 getHeader()메서드의 
	...
	*/
	...
	메서드는 헤더명을 동일하게 사용하더라도 덮어쓰기 되지 않고 각각 추가되게된다. 
	*/
